...
export function CategoriasPage() {
  ...
  const [filtros, setFiltros] = useState({ busca: '', tipo: 'TODOS', categoria: 'TODAS', dataInicio: '', dataFim: '' })
  ...
  const filtrados = lancamentos.filter(l => {
    const data = String(l.data_lancamento || '').slice(0, 10)
    const texto = `${l.nome || ''} ${l.categoriaNome || ''} ${l.origem || ''}`.toLowerCase()
    const tipoDaAba = tabAtiva === 'todas' || l.tipo === tabAtiva
    const tipoDoFiltro = filtros.tipo === 'TODOS' || l.tipo === filtros.tipo
    const categoria = filtros.categoria === 'TODAS' || String(l.id_categoria) === String(filtros.categoria)
    const inicio = !filtros.dataInicio || data >= filtros.dataInicio
    const fim = !filtros.dataFim || data <= filtros.dataFim
      ...
      <main className="container py-4">
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap" style={{ gap: 12 }}>
          ...
          <div className="d-flex gap-2 flex-wrap">
            ...
            <button type="button" className="btn btn-outline-primary" onClick={() => navegar('/relatorios')}>Relatorios</button>
          </div>
        </div>
        ...
        <FiltroBusca
          categorias={[...new Map(lancamentos.map(item => [item.id_categoria, { id_categoria: item.id_categoria, nome: item.categoriaNome }])).values()]}
