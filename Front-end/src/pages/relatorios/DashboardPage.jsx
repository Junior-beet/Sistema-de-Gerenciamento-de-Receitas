...
import { useFetchDados } from '../../hooks/useFetchDados.jsx'
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(erro, info) {
    console.error('Erro no painel:', erro, info)
  }
...
function DashboardConteudo({ usuario }) {
  const { dados, carregando, erro, recarregar } = useFetchDados(async () => {
    ...
    const categoriasArr = todasCategorias?.dados || []
    const idsCategorias = new Set(categoriasArr.map(c => c.id_categoria))

    const receitas = (todasReceitas?.dados || []).filter(r => idsCategorias.has(r.id_categoria))
    const despesas = (todasDespesas?.dados || []).filter(d => idsCategorias.has(d.id_categoria))

    return { categorias: categoriasArr, receitas, despesas }
  }, [])

  const [filtro, setFiltro] = useState({ busca: '', tipo: 'TODOS', categoria: 'TODAS', dataInicio: '', dataFim: '' })
  ...
  const receitasFiltradas = (dados?.receitas || []).filter(r => {
    ...
    return buscaMatch && tipoMatch && categoriaMatch && (!filtro.dataInicio || data >= filtro.dataInicio) && (!filtro.dataFim || data <= filtro.dataFim)
  })
  ...
  const despesasFiltradas = (dados?.despesas || []).filter(d => {
    ...
    const data = String(d.data_lancamento || '').slice(0, 10)
    ...
    <div>
      ...
      {carregando && !dados ? (
        ...
        <>
          ...
          <FiltroBusca categorias={dados.categorias} onFiltrar={handleFiltrar} />
