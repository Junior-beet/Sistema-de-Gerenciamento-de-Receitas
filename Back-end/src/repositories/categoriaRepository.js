...
const categoriaRepository = {
    ...
    selecionarPorIdEUsuario: async (id_categoria, id_usuario) => {
        const sql = `
            SELECT id_categoria, id_usuario, nome, tipo, cor, ordem
            FROM categorias
            WHERE id_categoria = ? AND id_usuario = ?
        `;
        const [rows] = await connection.execute(sql, [id_categoria, id_usuario]);
        return rows[0] || null;
    },
    ...
    deletar: async (id_categoria, id_usuario) => {
        const sql = 'DELETE FROM categorias WHERE id_categoria = ? AND id_usuario = ?';
        const [rows] = await connection.execute(sql, [id_categoria, id_usuario]);
        return rows;
    },
};
